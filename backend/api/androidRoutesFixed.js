...
const { mockAndroidReviews, mockAndroidAppInfo } = require('../utils/mockData');
...
router.get('/app-info', async (req, res) => {
  try {
    ...
    try {
      ...
      logger.info('API hatası nedeniyle mock app bilgileri kullanılıyor');
      return res.json({
        success: true,
        data: mockAndroidAppInfo
      });
    }
  } catch (error) {
    logger.error('Android uygulama bilgileri alınamadı', { error: error.message });
    
    // Kritik hata durumunda da mock veri kullan
    logger.info('Kritik hata nedeniyle mock app bilgileri kullanılıyor');
    return res.json({
      success: true,
      data: mockAndroidAppInfo
    });
...
router.get('/reviews', async (req, res) => {
  try {
    ...
    try {
      ...
      // Dönen objede yorum yoksa mock verilerini kullanalım
      if (!responseData || responseData.length === 0) {
        // Boş dönüş
        logger.info('Gerçek API yorumları boş döndü. Mock veriler kullanılıyor.');
        responseData = mockAndroidReviews;
        logger.info(`Mock olarak ${responseData.length} adet yorum eklendi.`);
      }
      ...
      // API hatası durumunda mock veri kullan
      logger.info('API hatası nedeniyle mock yorumlar kullanılıyor');
      return res.json({
        success: true,
        data: mockAndroidReviews
      });
    }
  } catch (error) {
    ...
    // Kritik hata durumunda da mock veri kullan
    logger.info('Kritik hata nedeniyle mock yorumlar kullanılıyor');
    return res.json({
      success: true,
      data: mockAndroidReviews
    });
  ...
  try {
    ...
    try {
      ...
      if (!allReviews || allReviews.length === 0) {
        // Boş dönüş
        logger.info(`Rating için gerçek API yorumları boş döndü. Mock veriler kullanılıyor.`);
        allReviews = mockAndroidReviews;
        logger.info(`Rating için mock olarak ${allReviews.length} adet yorum eklendi.`);
      }
      ...
      // API hatası durumunda mock veri kullan ve filtrele
      logger.info('API hatası nedeniyle mock yorumlar kullanılıyor');
      
      // Mock verileri kullan
      const filteredMockReviews = mockAndroidReviews.filter(review => Math.round(review.score) === rating);
    ...
    // Kritik hata durumunda da mock veri kullan ve filtrele
    logger.info('Kritik hata nedeniyle mock yorumlar kullanılıyor');
